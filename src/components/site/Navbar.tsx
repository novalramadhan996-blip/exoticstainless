  ...
  return (
    <header
      className={`fixed inset-x-0 top-0 z-50 transition-all duration-300 ${
        solid
          ? "border-b border-border/60 bg-background/95 backdrop-blur-xl shadow-soft"
        ...
        <nav className="hidden items-center gap-1 lg:flex">
          {NAV_LINKS.map((link) => (
            <Link
              ...
              className="rounded-lg px-3.5 py-2 text-sm font-semibold text-foreground/80 transition-colors hover:text-primary"
              ...
              {NAV_LINKS.map((link, i) => (
                ...
                >
                  <Link
                    ...
                    className="block rounded-lg px-3 py-3 text-base font-semibold text-foreground/80 hover:bg-muted hover:text-primary"
                    activeProps={{ className: "text-primary" }}
                    activeOptions={{ exact: link.to === "/" }}
                  >
                    {link.label}
